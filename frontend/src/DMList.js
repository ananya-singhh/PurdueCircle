...
const DMList = () => {
    ...
    return(
        ...
        <Card>
            <Card.Header>
                <Button variant="primary" style={{float: "right"}} onClick={navigate(-1)}>Back</Button>
                <h1>My Messages</h1>
            </Card.Header>
            <Card.Body>
                ...
                <ListGroup.Item action variant="light" onClick={() => navigate('/Profile/DMPage/'+user['username']+'/'+item)}>{item.split('_')[0] !== user['username'] ? item.split('_')[0] : item.split('_')[1]}</ListGroup.Item>
                ))}
